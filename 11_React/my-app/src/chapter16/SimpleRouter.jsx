...
import PalcePage from './pages/PalcePage';
...
// 페이지를 나누는 법
// 1) 리액트 미사용 시
// MPA(Multi Page Application) : 각 페이지별 html 문서를 만듦.
// /list 경로로 접속하면 해당 list.html 파일 보내줌

// 2) 리액트 사용 시
// SPA(Single Page Application) : index.html 파일 하나만 존재한다
// List 컴포넌트를 만듦
  ...
  return (
    <BrowserRouter>
      <Routes>
        ...
        <Route path='/places' element={<PalcePage/>}/>
        ...
        {/* 부모 안에 자식들을 렌더링해서 보여줌(어디에 보여줄지는 부모안에서 Outlet 컴포넌트로 저장) */}
        {/* 활용 예: 헤더, 푸터 사이에 메인 영역 등 */}
        {/* Newsted 안에 Newsted 가능 */}
        ...
        {/* URL 파라미터는 주소의 경로에 유동적인 값을 넣는 형태고, 
          쿼리스트링은 주소의 뒷부분에 ? 문자열 이후 key=value 형태로 값을 정의하여 &로 구분하는 형태 */}

        {/* 주로 URL 파라미터는 ID 또는 이름을 사용하여 특정 데이터를 조회할 떄 사용하고, 
