...
function ApiRequest(props) {
  ...
  const handleRequestById = (id) => {
    // jsonplaceholder(테스트용 API 서버 - 테스트용 서버) 에서 제공하는 테스트용 API를 호출
    axios.get(`https://jsonplaceholder.typicode.com/photos/${id}`) // 프로미스를 반환
      .then((response) => {
  ...
  return (
    <>
      <div>
        <button onClick={() => { handleRequestById(800) }}>Promis/then 불러오기</button>
        <button onClick={() => setData(null)}>비우기</button>
        <button onClick={() => { handleRequestByIdAsync(800) }}>async/await 불러오기</button>
      </div>
      {data && (
        <>
...
// 데이터를 보낼때(추가, 수정, 삭제)는 POST 방식을 사용
// 그 외 PUT, PATCH, DELETE 등도 있음

// 자바스크립트를 이용하여 서버쪽 데이터 요청 가능 = Ajax 사용하면 됨(비동기 방식, 새로고침 안됨)
