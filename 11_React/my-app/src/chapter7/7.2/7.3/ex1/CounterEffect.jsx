...
function CounterEffect(props) {
  ...
  return (
    <>
      <p>총 {count}번 클릭했습닌다.</p>
      <button onClick={()=>{
        setCount(count + 1)

      document.title = `You clicked ${count} times`
      // 차이점
      // 1. 초기에 타이틀이 안바뀌어 있음 (왜? 처음 마운트 시 실행이 안되어서..)
      // 2. 버튼 클릭 시 카운트값 반영이 제대로 안됨 (왜? set함수는 비동기 함수.. )

      // onClick과 useEffect는 실행 시점이 다르기에 동작이 다를 수 밖에 없다(완전히 다른 용도)
      // onClick 속성으로 넣어준 콜백 함수는 "클릭 시 호출"되는 함수
      // useEffect 내부에 있는 콜백 함수는 "렌더링 시 호출"되는 함수
